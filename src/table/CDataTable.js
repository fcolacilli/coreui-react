...
const CDataTable = (props) => {
	...
	const isSortable = (i) => {
		...
		return (
			sorter && (!fields || fields[i].sorter !== false) && isDataColumn
		);
	};
	...
	const changeSort = (column, index) => {
		...
		if (!sorter || !sorter.resetable) {
			...
			state.column =
				columnRepeated && state.asc === false ? null : column;
		}
	...
	const tableFiltered = useMemo(() => {
		...
		items = items.filter((item) => {
			return !!itemsDataColumns.find((key) =>
	...
	const tableFilterData = {
		...
		placeholder:
			(tableFilter && tableFilter.placeholder) || "type string...",
	};

	const paginationSelect = {
		label:
			(itemsPerPageSelect && itemsPerPageSelect.label) ||
			"Items per page:",
